...
<<setup, include=FALSE>>=
library(knitr)
# Only evaluate code that needs spdep when it is actually installed, so the
# vignette builds cleanly in environments where Suggests are unavailable.
has_spdep <- requireNamespace("spdep", quietly = TRUE)
opts_chunk$set(eval = has_spdep, echo = TRUE, fig.width = 5, fig.height = 4)
@
...
\begin{itemize}
  ...
  \item \textbf{Reproducibility.} Supplying \texttt{iseed} with
    \texttt{n.cores = 1L} makes permutation inference reproducible. Dynamic
    scheduling means multi-core permutation results may differ between runs.
\end{itemize}
